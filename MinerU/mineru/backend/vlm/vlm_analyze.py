...
from ...utils.enum_class import ImageType, ModelPath
...
def _vlm_path_for_pretrained(model_path: str | None) -> str | None:
    """mineru.json stores snapshot_download() paths; passing them to transformers.from_pretrained
    can trigger HFValidationError in recent huggingface_hub (path validated as repo id). Use the
    repo id so the Hub resolves from HF_HOME cache."""
    if not model_path:
        return model_path
    norm = model_path.replace("\\", "/")
    if "/hub/models--" in norm and "/snapshots/" in norm:
        ms = os.getenv("MINERU_MODEL_SOURCE", "huggingface")
        if ms == "modelscope":
            return ModelPath.vlm_root_modelscope
        return ModelPath.vlm_root_hf
    return model_path
        ...
        **kwargs,
        ...
        if key not in self._models:
            ...
            if backend == "transformers":
                ...
                device = get_device()
                load_id = _vlm_path_for_pretrained(model_path)
                ...
                model, processor = mlx_load(_vlm_path_for_pretrained(model_path))
            else:
