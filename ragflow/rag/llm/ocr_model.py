...
class MinerUOcrModel(Base, MinerUParser):
    ...
    def __init__(self, key: str | dict, model_name: str, **kwargs):
        ...
        def _resolve_config(key: str, env_key: str, default=""):
            # Env wins when set (e.g. MINERU_APISERVER=http://mineru-api:8000 in Docker Compose)
            # so host.docker.internal in saved UI config does not break same-network MinerU.
            env_val = os.environ.get(env_key)
            if env_val is not None and str(env_val).strip() != "":
                return env_val
            return config.get(key, config.get(env_key, default))
    ...
    def parse_pdf(self, filepath: str, binary=None, callback=None, parse_method: str = "raw", **kwargs):
        backend = kwargs.pop("mineru_backend", None) or self.mineru_backend
        server_url = kwargs.pop("mineru_server_url", None) or self.mineru_server_url
        ok, reason = self.check_available(backend=backend, server_url=server_url)
        if not ok:
