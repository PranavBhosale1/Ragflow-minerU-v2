...
from deepdoc.parser.mineru_parser import MinerUBackend
...
# layout_recognize values like "vlm-transformers@MinerU" put the MinerU API backend in the
# @ prefix slot; that must not be mistaken for a tenant OCR model name.
_MINERU_BACKEND_VALUE_SET = frozenset(m.value for m in MinerUBackend)
...
def by_mineru(
    ...
    **kwargs,
    ...
    pdf_parser = None
    mineru_backend_override = None
    if mineru_llm_name and mineru_llm_name in _MINERU_BACKEND_VALUE_SET:
        mineru_backend_override = mineru_llm_name
        mineru_llm_name = None
    ...
    if tenant_id:
        ...
        if mineru_llm_name:
            try:
                ...
                parse_kw = dict(kwargs)
                if mineru_backend_override:
                    parse_kw["mineru_backend"] = mineru_backend_override
    ...
    if callback:
        callback(-1, "MinerU not found. Add MinerU under Model Providers or set MINERU_APISERVER (e.g. http://mineru-api:8000 in Docker).")
    return None, None, None
