...
def _embedding_batch_encode(mdl, txts):
    """Encode texts; on batch failure retry one-by-one to isolate bad inputs."""
    truncated = [truncate(c, mdl.max_length - 10) for c in txts]
    try:
        return mdl.encode(truncated)
    except Exception as e:
        if len(truncated) <= 1:
            raise
        logging.warning("Embedding batch failed (%s); retrying per text.", str(e)[:240])
        rows = []
        total_c = 0
        for t in truncated:
            v, c = mdl.encode([t])
...
async def embedding(docs, mdl, parser_config=None, callback=None):
    ...
    def batch_encode(txts):
        nonlocal mdl
        return _embedding_batch_encode(mdl, txts)
    ...
    vects = np.asarray(vects, dtype=np.float64)
    if not np.isfinite(vects).all():
        logging.warning("Embedding vectors contained NaN or Inf after combine; sanitizing")
        vects = np.nan_to_num(vects, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if not any([re.match(r"q_[0-9]+_vec", k) for k in keys]):
        try:
            ...
            def batch_encode(txts):
                ...
                return _embedding_batch_encode(embedding_model, txts)
