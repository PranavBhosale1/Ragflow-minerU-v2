...
class LLMBundle(LLM4Tenant):
    ...
    @staticmethod
    def _sanitize_embedding_array(embeddings):
        arr = np.asarray(embeddings, dtype=np.float64)
        if not np.isfinite(arr).all():
            logging.warning("Embedding output contained NaN or Inf; values replaced with 0 for JSON storage")
    ...
    def encode(self, texts: list):
        ...
        embeddings = self._sanitize_embedding_array(embeddings)
    ...
    def encode_queries(self, query: str):
        ...
        emd = self._sanitize_embedding_array(emd)
        if self.model_config["llm_factory"] == "Builtin":
